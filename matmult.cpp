...
/*void matrixInput(ifstream file, vector< vector<int> > &matrix, int m, int n) {
	int entry;
	int i = 0;
	int j = 0;

	// Taking input of Matrices from param files for Matrix
	while (1) {
		file >> entry;
		matrix[i][j] = entry;
		j++;
...
void displayMatrix(vector< vector<int> > &matrix, int m, int n) {
	for (int i = 0; i < m; i++) {
		for (int j = 0; j < n; j++) {
